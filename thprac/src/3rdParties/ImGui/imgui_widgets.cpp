...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible)
        flags |= static_cast<int>(ImGuiTreeNodeFlags_AllowItemOverlap) | static_cast<int>(ImGuiTreeNodeFlags_ClipLabelForTrailingButton);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, bool text_wrap, bool legacy_columns)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (text_wrap) {
        wrap_width = CalcWrapWidthForPos(window->DC.CursorPos, 0.0f) ;
        label_size = CalcTextSize(label, 0, false, wrap_width);
    ...
    if (span_all_columns && legacy_columns) {
        bb.Min.x -= ImGui::GetStyle().ItemSpacing.x;
        bb.Max.x += ImGui::GetStyle().ItemSpacing.x;
    }
    ...
    if (text_wrap) {
        RenderTextWrapped(text_min, label, nullptr, wrap_width);
    } else {
